...
public record GenerateDowithTaskSuccessImageUploadPresignedUrlsResDto(
        @Schema(
                        description = "업로드 완료 후 노출될 공개 이미지 URL 목록 (요청 파일명 순서와 대응)",
                        example = "[\"https://example.com/success1.jpg\"]")
                List<String> publicImageUrls,
        @Schema(
                        description = "실제 업로드에 사용할 presigned URL 목록",
                        example = "[\"https://example.com/success1.jpg?X-Amz-Signature=...\"]")
