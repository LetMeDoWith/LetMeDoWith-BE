...
public record RetrieveFeedbackAvailableDowithTasksResDto(
        @Schema(description = "잔소리 가능 두윗 태스크 목록") List<RetrieveFeedbackAvailableDowithTaskDto> dowithTasks) {
    ...
    public record RetrieveFeedbackAvailableDowithTaskDto(
            @Schema(description = "태스크 ID", example = "1") Long id,
            @Schema(description = "회원 ID", example = "test_member_id") String memberId,
