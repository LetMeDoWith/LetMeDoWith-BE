...
public record RetrieveNoticesResDto(@Schema(description = "공지사항/이벤트 목록") List<RetrieveNoticeResDto> notices) {
    ...
    public record RetrieveNoticeResDto(
            ...
            @Schema(description = "공지/이벤트 타입", example = "\"NOTICE\" | \"EVENT\"") NoticeType type,
        ...
        public static RetrieveNoticeResDto from(RetrieveNoticeResult result) {
            return new RetrieveNoticeResDto(
                    result.id(), result.title(), result.type(), result.createdAt(), result.thumbnailImageUrl());
        }
    }
}
