...
public record RetrieveTaskFeedbackAggregateResDto(
        @Schema(description = "집계된 잔소리 템플릿 메시지", example = "집계된 잔소리 템플릿 메시지") List<AggregateDto> aggregates) {
    public static RetrieveTaskFeedbackAggregateResDto from(RetrieveTaskFeedbackAggregateResult result) {
        List<AggregateDto> aggregates = result.aggregates().stream()
                .map(dto -> new AggregateDto(dto.feedbackTemplateId(), dto.count()))
                .toList();
        return new RetrieveTaskFeedbackAggregateResDto(aggregates);
    }
