...
public record RetrieveRankingsResult(List<RetrieveRankingResult> rankings) {
    public static RetrieveRankingsResult from(List<RankingsQueryDto> rankings) {
        return new RetrieveRankingsResult(rankings.stream()
                .map(RetrieveRankingResult::from)
                .toList());
    }
    
    public record RetrieveRankingResult(Integer year,
    Integer month,
    Integer week,
    Long topicId,
    String topicTitle,
    ...
    String profileImageUrl) {
        public static RetrieveRankingResult from(RankingsQueryDto ranking) {
            return new RetrieveRankingResult(ranking.year(), ranking.month(), ranking.week(), ranking.topicId(), ranking.topicTitle(), ranking.currentRank(), ranking.previousRank(), ranking.memberId(), ranking.nickname(), ranking.profileImageUrl());
        }
    }
}
