...
public record RetrieveNoticesResult(Long totalCount, List<RetrieveNoticeResult> notices) {
    ...
    public static RetrieveNoticesResult from(
        Long totalCount,
        List<NoticeQueryDto> noticeQueryDtos) {
        return new RetrieveNoticesResult(totalCount,
            noticeQueryDtos.stream().map(RetrieveNoticeResult::from).toList());
    }

    public record RetrieveNoticeResult(Long id,
                                       String title,
                                       NoticeType type,
        ...
        public static RetrieveNoticeResult from(NoticeQueryDto queryDto) {
            return new RetrieveNoticeResult(queryDto.id(), queryDto.title(), queryDto.type(),
                queryDto.createdAt(),
