...
public class NudgeTodoTaskStartNotifyTasklet implements Tasklet {
    ...
    public @Nullable RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
        ...
        // 시작 시각이 (실행 시각 + 10분)인 TodoTask들의 todoTaskId, todoTaskTitle, member_id 조회하기
        ...
        LocalDateTime windowEnd = windowStart.plusMinutes(JOB_INTERVAL_MINUTES).minusSeconds(10);

        List<NudgeTodoTaskTarget> nudgeTargets = findNudgeTargets(windowStart, windowEnd);
        ...
        List<String> receiverMemberIds =
                nudgeTargets.stream().map(NudgeTodoTaskTarget::memberId).toList();
        List<Map<String, String>> bodyParams = nudgeTargets.stream()
                .map(target -> Map.of(
                        "todoTaskId", String.valueOf(target.todoTaskId()),
                        "todoTaskTitle", target.todoTaskTitle()))
                .toList();

        SendNotificationResult sendNotificationResult = notificationSendService.sendNotifications(
                NotificationTemplateCode.NUDGE_TODO_START, receiverMemberIds, null, bodyParams, null);

        if (!sendNotificationResult.failedMemberIds().isEmpty()) {
            for (NudgeTodoTaskTarget target : nudgeTargets) {
                if (sendNotificationResult.failedMemberIds().contains(target.memberId())) {
                    log.error(
                            "Todo 시작 재촉 알림 발송 실패 - memberId: {}, todoTaskId: {}",
                            target.memberId(),
                            target.todoTaskId());
                }
            }
    ...
    /**
     * windowStart ~ windowEnd 구간의 TodoTask를 조회한다.
     * date와 start_time이 분리된 컬럼이라, 구간이 자정을 넘어가면 날짜별로 나눠 조회한 뒤 합친다.
     */
    private List<NudgeTodoTaskTarget> findNudgeTargets(LocalDateTime windowStart, LocalDateTime windowEnd) {
        ...
        List<NudgeTodoTaskTarget> nudgeTargets = new ArrayList<>();
    ...
    private List<NudgeTodoTaskTarget> queryTargets(LocalDate date, LocalTime startTimeFrom, LocalTime startTimeTo) {
        return jdbcTemplate.query(
                """
                        SELECT id         AS todo_task_id,
                               title      AS todo_task_title,
                        ...
                        """,
                (rs, rowNum) -> new NudgeTodoTaskTarget(
                        rs.getLong("todo_task_id"), rs.getString("todo_task_title"), rs.getString("member_id")),
                TodoTaskStatus.WAIT.code,
                date,
                startTimeFrom,
                startTimeTo);
    }

    public record NudgeTodoTaskTarget(Long todoTaskId, String todoTaskTitle, String memberId) {}
}
