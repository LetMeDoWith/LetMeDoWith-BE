...
public class FeedbackKingJobScheduler {
    ...
    public void feedbackKingRankingJob() {
        ...
        JobParameters jobParameters = new JobParametersBuilder()
                ...
                .toJobParameters();
        try {
            JobExecution jobExecution = jobLauncher.run(feedbackKingRankingJob, jobParameters);
            if (jobExecution.getStatus() != BatchStatus.COMPLETED) {
                log.error(
                        "feedbackKingRankingJob finished with non-completed status. status={}, executionDateTime={}",
                        jobExecution.getStatus(),
                        executionDateTime);
                throw new IllegalStateException("feedbackKingRankingJob failed. status=" + jobExecution.getStatus());
            }
        } catch (Exception e) {
            log.error("Failed to run monthlyFeedbackKingRankingJob", e);
            throw new IllegalStateException("Failed to run monthlyFeedbackKingRankingJob", e);
        }
    }
}
