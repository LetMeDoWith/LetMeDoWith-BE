...
public class SocialAuthMemberService {
     ...
     * @return 임시 멤버 객체
     */
    public Member createSocialAuthenticatedMember(
            SocialProvider provider, String subject, MemberRepository memberRepository) {
        Member socialAuthenticatedMember = memberRepository.save(Member.ofSocialAuthenticated(subject));
        memberRepository.saveSocialAccount(MemberSocialAccount.of(socialAuthenticatedMember, provider));
