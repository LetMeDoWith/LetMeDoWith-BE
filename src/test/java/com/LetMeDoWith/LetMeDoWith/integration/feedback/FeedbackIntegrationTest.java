...
public class FeedbackIntegrationTest extends AbstractIntegrationTest {
    ...
    private static final String TEST_LANGUAGE = "KR";
    @Autowired private DowithTaskJpaRepository dowithTaskRepo;
    @Autowired private TaskFeedbackTemplateJpaRepository templateRepo;
    @Autowired private TaskFeedbackTemplateMessageJpaRepository templateMsgRepo;
    ...
    void createFeedback_success() throws Exception {
        CreateDowithFeedbackReqDto req =
        ...
        assertThat(feedback.taskFeedbackTemplateId()).isEqualTo(template.getId());
    ...
    void retrieveFeedbackByTaskId_success() throws Exception {
        ...
        RetrieveTaskFeedbackResult result =
                this.readResponse(content, RetrieveTaskFeedbackResult.class);
        assertThat(result.feedbacks()).isNotEmpty();
        DowithTaskFeedbackQueryDto feedback = result.feedbacks().get(0);
        assertThat(feedback.dowithTaskId()).isEqualTo(dowithTask.getId());
        assertThat(feedback.taskFeedbackTemplateId()).isEqualTo(template.getId());
    }
    ...
    void checkFeedback_success() throws Exception {
        ...
        var afterResult = this.readResponse(afterCheckContent, RetrieveTaskFeedbackResult.class);
        var afterFeedback = afterResult.feedbacks().get(0);
