...
public class CreateDowithTaskIntegrationTest {
    ...
    private ResultActions requestCreateDowithTask(
            AccessToken accessToken, CreateDowithTaskReqDto requestBody) throws Exception {
        LinkedMultiValueMap<String, String> headerMap = new LinkedMultiValueMap<>();
        headerMap.add("AUTHORIZATION", "Bearer" + accessToken.getToken());

        return mockMvc
                .perform(
                        MockMvcRequestBuilders.post(BASE_URL + CREATE_DOWITH_TASK)
    ...
    void createDowithTask() throws Exception {
        ...
        resultActions
                ...
                .andExpect(
                        jsonPath("$.data.dowithTaskDtos[0].date")
                                .value(DateTimeUtil.toFormatString(startDateTime.toLocalDate())))
                .andExpect(
                        jsonPath("$.data.dowithTaskDtos[0].startTime")
                                .value(DateTimeUtil.toFormatString(startDateTime.toLocalTime())))
    ...
    void createDowithTaskWithRoutine() throws Exception {
        ...
        for (int i = 0; i < targetDates.size(); i++) {
            resultActions
                    ...
                    .andExpect(
                            jsonPath("$.data.dowithTaskDtos[" + i + "].taskCategoryId")
                                    .value(requestBody.taskCategoryId()))
                    .andExpect(jsonPath("$.data.dowithTaskDtos[" + i + "].title").value(requestBody.title()))
                    .andExpect(
                            jsonPath("$.data.dowithTaskDtos[" + i + "].status")
                                    .value(DowithTaskStatus.WAIT.getCode()))
                    .andExpect(
                            jsonPath("$.data.dowithTaskDtos[" + i + "].date")
                                    .value(DateTimeUtil.toFormatString(targetDates.get(i))))
                    .andExpect(
                            jsonPath("$.data.dowithTaskDtos[" + i + "].startTime")
                                    .value(DateTimeUtil.toFormatString(startDateTime.toLocalTime())))
                    .andExpect(jsonPath("$.data.dowithTaskDtos[" + i + "].isRoutine").value(Boolean.TRUE))
                    .andExpect(
                            jsonPath("$.data.dowithTaskDtos[" + i + "].routineDates")
                                    .value(
                                            new IsEqual<>(
                                                    List.of(
                                                            DateTimeUtil.toFormatString(targetDates.get(0)),
