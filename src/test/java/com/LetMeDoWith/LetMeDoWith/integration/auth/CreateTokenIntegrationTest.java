...
public class CreateTokenIntegrationTest extends AbstractIntegrationTest {
    ...
    private void setupMockBehaviors() {
        ...
        when(oidcIdTokenProvider.getVerifiedOidcIdToken(any(SocialProvider.class), anyString()))
            .thenAnswer(invocation -> {
                ...
                when(mockClaims.getExpiration()).thenReturn(
    ...
    void createToken_firstTimeLogin_returnsSignupToken() throws Exception {
        // given
        CreateTokenReqDto requestBody = new CreateTokenReqDto(SocialProvider.KAKAO,
            "new_user_token");
        ...
        // 새로운 멤버가 생성되었는지 확인
        assertThat(testMemberJpaRepository.findAll()).hasSize(
            5); // requestMember + 3개 테스트 멤버 + 새로 생성된 멤버
        // 새로 생성된 TaskSummary 확인
        assertThat(testTaskSummaryJpaRepository.findAll()).hasSize(
            2); // requestMember TaskSummary + 새로 생성된 것
    }
    ...
    void createToken_suspendedMember_throwsException() throws Exception {
        ...
        resultActions
            ...
            .andExpect(jsonPath("$.statusCode").value(
                FailResponseStatus.LOGIN_ATTEMPTED_SUSPENED.getStatusCode()));
    }
    ...
    void createToken_invalidIdToken_throwsException() throws Exception {
        ...
        resultActions
            .andExpect(status().is4xxClientError())
            .andExpect(
                jsonPath("$.statusCode").value(FailResponseStatus.INVALID_TOKEN.getStatusCode()));
    }

    private ResultActions performCreateTokenRequest(CreateTokenReqDto requestBody)
        throws Exception {
        return this.request(MockMvcRequestBuilders.post(BASE_URL + CREATE_TOKEN_URL)
