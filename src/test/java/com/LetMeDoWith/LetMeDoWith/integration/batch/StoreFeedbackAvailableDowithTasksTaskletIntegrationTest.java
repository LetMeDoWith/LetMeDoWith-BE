...
class StoreFeedbackAvailableDowithTasksTaskletIntegrationTest extends AbstractIntegrationTest {
    ...
    void testExecute_Integration() throws Exception {
        ...
        // 1. 상세 정보 저장 호출 확인
        verify(redisOperator).putHashes(eq(CachePolicy.DOWITH_TASK), eq(tasks), any());

        // 2. ID 목록 저장 호출 확인 (Atomic Rename 로직 검증)
        // pushRightAll -> rename 순서로 호출되었는지 확인
        verify(redisOperator).pushRightAll(eq(CachePolicy.DOWITH_TASK_IDS), anyString(), anyList());
        verify(redisOperator).rename(eq(CachePolicy.DOWITH_TASK_IDS), anyString(), eq(""));
    }
}
