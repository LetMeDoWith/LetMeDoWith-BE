...
@SpringBatchTest
@ActiveProfiles("test")
class NudgeTodoTaskStartNotifyTaskletTest {
    ...
    @Autowired
    private Job nudgeTodoTaskStartJob;
    ...
    @BeforeEach
    void setUp() {
        jobLauncherTestUtils.setJob(nudgeTodoTaskStartJob);
        ...
        notificationTemplate = notificationTemplateJpaRepository.save(NotificationTemplate.of(
                NotificationTemplateCode.NUDGE_TODO_START,
                NotificationType.NORMAL,
                "할 일 시작 10분 전이에요",
                "{{todoTaskTitle}} 시작이 10분 남았어요!",
                "letmedowith://todo"));
    }
    ...
    @Test
    @DisplayName("[SUCCESS] 시작까지 10분 남은 WAIT 상태의 TodoTask에 알림을 보낸다")
    void nudgeTodoTaskStart_success() throws Exception {
        // given
        ...
        todoTaskJpaRepository.save(TodoTask.of(
                member.getId(), null, "아침 운동", targetStartDateTime.toLocalDate(), targetStartDateTime.toLocalTime()));
        ...
        assertThat(savedNotification.get().getBody()).isEqualTo("아침 운동 시작이 10분 남았어요!");
        assertThat(savedNotification.get().getDeepLink()).isEqualTo("letmedowith://todo");
        assertThat(savedNotification.get().getNotificationTemplateCode())
                .isEqualTo(NotificationTemplateCode.NUDGE_TODO_START.getCode());
    }

    @Test
    @DisplayName("[FAIL] 시작까지 10분 이상 남은 TodoTask는 알림 대상에서 제외된다")
    void nudgeTodoTaskStart_outsideWindow_notNotified() throws Exception {
        // given
        LocalDateTime executionDateTime = LocalDateTime.of(2026, 1, 1, 9, 0, 0);
    ...
    @Test
    @DisplayName("[FAIL] 이미 완료(SUCCESS)된 TodoTask는 알림 대상에서 제외된다")
    void nudgeTodoTaskStart_alreadySuccess_notNotified() throws Exception {
        // given
        LocalDateTime executionDateTime = LocalDateTime.of(2026, 1, 1, 9, 0, 0);
        LocalDateTime targetStartDateTime = executionDateTime.plusMinutes(12);

        TodoTask completedTodoTask = TodoTask.of(
                member.getId(),
                null,
                "이미 끝난 할 일",
                targetStartDateTime.toLocalDate(),
                targetStartDateTime.toLocalTime());
        completedTodoTask.success();
        todoTaskJpaRepository.save(completedTodoTask);
    ...
    @Test
    @DisplayName("[SUCCESS] 윈도우가 자정을 넘어가도 다음날 TodoTask를 정상적으로 조회해 알림을 보낸다")
    void nudgeTodoTaskStart_midnightRollover_success() throws Exception {
        // given
        ...
        Optional<Notification> savedNotification = notificationJpaRepository.findByMemberId(member.getId());
        assertThat(savedNotification).isPresent();
        assertThat(savedNotification.get().getBody()).isEqualTo("자정 넘어 시작하는 할 일 시작이 10분 남았어요!");
        assertThat(savedNotification.get().getDeepLink()).isEqualTo("letmedowith://todo");
    }
}
